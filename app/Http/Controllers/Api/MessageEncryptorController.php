...
class MessageEncryptorController extends Controller
{
    ...
    public function __construct(protected CryptoService $cryptoService, protected EncryptRequestService $encryptRequestService, protected TokenPermissionService $tokenPermissionService)
    {
    }
    ...
    public function index(Request $request)
    {
        ...
        if(!$this->tokenPermissionService->getTokenPermission($request, "*")){
            return response()->json(HTTPResponseEnum::getUnathorizedMessage(), HTTPResponseEnum::FORBIDDEN);
        };
    ...
    {
        $this->user = Auth::user();
        if(!$this->tokenPermissionService->getTokenPermission($request, "read")){
            return response()->json(HTTPResponseEnum::getUnathorizedMessage(), HTTPResponseEnum::FORBIDDEN);
        };
        $document = $this->user->documents()->where('uuid', $uuid)->get();
        if(count($document) === 0){
            return response()->json(HTTPResponseEnum::getNotFoundMessage('Get an encypted document', $uuid), HTTPResponseEnum::NOT_FOUND);
        }
